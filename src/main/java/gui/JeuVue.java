 ...
 */
public class JeuVue extends Scene implements KeyListener {
    protected Jeu model;
    ...
    public JeuVue(Jeu m, boolean visible) throws IOException {
        super(visible);
        Main.getView().setTitle("Plateau Tuile");

        model = m;
        gridTuile = new GridTuile(model);
        // controleur=new Controleur(model, this);
        // TODO: construire l'interface graphique (avec la tuileTemple du joueur0 comme
        // la tuile initiale)
        ...
        afficherPossibleTuilePosition();
        afficherPossiblePionPosition();
        afficherPossibleDestination();
        setupJeuVue();
    ...
    public void creerPlateau() {
        while (true) {
            // for(int i=0; i<model.getJoueurs().size(); i++) System.out.println("affiche
            // joueurs:"+model.getJoueurs().get(i));
            for (int i = 0; i < model.getJoueurs().size(); i++) {
                ...
                ArrayList<Coordonnee> emplacementsPossibles = model.getPlateau()
                        .canPlaceLocations(ancienneTuileCoordonnee);
                ...
                while (emplacementsPossibles.isEmpty()) {
                    emplacementsPossibles = model.getPlateau().canPlaceLocations(ancienneTuileCoordonnee.getX() + dx,
                            ancienneTuileCoordonnee.getY());
                    dx--;
                }
    ...
    public void jouer() {
        ...
        // Surement temporaire : permet de placer les pions automatiquement après que
        // les tuiles ont toutes été posées
    ...
    public ArrayList<Coordonnee> afficherPossibleTuilePosition() {
        ArrayList<Coordonnee> locationsPossibles = model.getPlateau()
                .canPlaceLocations(model.getTuileCourant().getLocationInGridTuile());
    ...
    /**
     * TODO: dessiner un cercle rouge (indiquant la locations possible) dont le
     * centre est sur la coordonn茅 donn茅
     * taille du cercle: un peu plus grand que la taille du cercle dans le centre
     * des certaines tuiles
     */
    ...
    /**
     * TODO: dessiner un cercle blanc (indiquant la location choisie) dont le centre
     * est sur la coordonn茅 donn茅
     * taille du cercle: un peu plus grand que la taille du cercle dans le centre
     * des certaines tuiles
     */
    ...
    private void glisserTuile(Tuile tuile, int dx, int dy) {
        ...
        if (!model.getPlateau().placeTuileForce(tuile, x + dx, y + dy)) {
            if (dx > 0) {
                glisserTuile(tuile, dx + 1, dy);
            } else if (dx < 0) {
                glisserTuile(tuile, dx - 1, dy);
            } else if (dy > 0) {
                glisserTuile(tuile, dx, dy + 1);
            } else if (dy < 0) {
                glisserTuile(tuile, dx, dy - 1);
            }
        }
        ...
        switch (e.getKeyChar()) {
            ...
            case ' ': // verifier le choix
                if (model.getJeuEtat() == JeuEtat.CHOOSING_TUILE_LOCATION) {
                    ...
                    model.getPlateau().removeTuileBrutForce(model.getTuileCourant().getLocationInGridTuile()); // On
                                                                                                               // enlève
                                                                                                               // la
                                                                                                               // tuile
                                                                                                               // courante
                                                                                                               // du
                                                                                                               // plateau
                    if (model.getPlateau().placeTuileContraint(model.getTuileCourant(),
                            model.getTuileCourant().getLocationInGridTuile())) { // Et on tente de la replacer, mais
                        ...
                        model.getPlateau().placeTuileForce(model.getTuileCourant(),
                                model.getTuileCourant().getLocationInGridTuile());
                    }
                } else if (model.getJeuEtat() == JeuEtat.ROTATING_TUILE) {
                    // TODO:programmez ici pour effacer tous les cercles
                    model.setJeuEtat(JeuEtat.CONTINUE);
                    afficherInstruction(
                            "Tappez 鈫鈫pour chosir un pion 脿 d茅placer. Tappez SPACE pour v茅rifier la choix ou passer votre tour.");
                }
                ...
                // model.setJeuEtat(JeuEtat.CONTINUE);
                // afficherInstruction("Tappez 鈫鈫pour chosir la location. Tappez SPACE pour
                // v茅rifier la choix ou passer votre tour.");
                // }
                // else if(model.getJeuEtat()==JeuEtat.PLACING_PION){
                // //TODO:programmez ici pour effacer tous les cercles
                // //TODO: renouveler la vue des pions
                // model.getPlateau().placerPionForce(model.getPionCourant(),
                // locationsPossibles.get(indexChoisi));
                // model.setJeuEtat(JeuEtat.CONTINUE);
                // afficherInstruction("Tappez 鈫鈫pour chosir un pion 脿 passer la perle. Tappez
                // SPACE pour v茅rifier la choix ou passer votre tour.");
                // }
    ...
    public void setupJeuVue() throws IOException {
        Jeu model = new Jeu(2);
        JeuVue jeuVue = new JeuVue(model, true);
        jeuVue.lancer();
